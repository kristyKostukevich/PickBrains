...
export class PersonListComponent implements OnInit {
  ...
  date: Date;
  urlSearch: string;
  urlDefault: string = 'http://192.168.43.8:1488/api/candidates/search';
  ...
  ngOnInit() {
    this.httpService.postData({skip: 0, amount: this.countOfElements}, 'http://192.168.43.8:1488/api/candidates')
      .subscribe((res) => {
  ...
  sendSearchLine(event) {
    ...
    if (!event) {
      ...
      this.urlSearch = `http://192.168.43.8:1488/api/candidates/search?q=${event}`;
    }
  ...
  send(event) {
    ...
    this.httpService.postData(this.body, 'http://192.168.43.8:1488/api/candidates')
      .subscribe((res) => {
        this.persons = res.json();
        if (this.isLastItem())
          this.persons.pop();
