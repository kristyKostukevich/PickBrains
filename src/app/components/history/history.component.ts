...
export class HistoryDatabase {
  ...
  constructor(private httpService: HttpService) {
    this.httpService.getData('http://192.168.43.135:1337/api/history/?skip=0&capacity=10')
      .subscribe((res) => {
  ...
  onClick(event) {
    ...
    this.httpService
      .getData(`http://192.168.43.135:1337/api/history/?skip=${skip}&capacity=${event.pageSize}`)
      .subscribe((res) => {
        const interviews: HistoryData[] = res.json().slice(1);
        interviews.forEach((interview) => {
          interview.date = new Date(interview.date);
